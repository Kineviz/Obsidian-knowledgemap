...
# Configure logging to suppress polling endpoint logs
class PollingLogFilter(logging.Filter):
    """Filter out access logs for job status polling requests"""
    def filter(self, record):
        # Suppress logs for /api/jobs/ polling requests
        # Uvicorn access logs format: "GET /api/jobs/xxx HTTP/1.1" 200
...
class RemoveTagsRequest(BaseModel):
    folder: Optional[str] = None  # If None, remove from entire vault


@app.post("/api/tasks/{tag}/remove-tags")
async def remove_task_tags(tag: str, request: RemoveTagsRequest):
    """Remove all tags generated by a task from notes
    
    For single-tag tasks: removes the task's tag
    For multi-tag tasks: removes all tags in the task's tag_schema
    
    If folder is specified, only removes from that folder.
    If folder is None, removes from entire vault.
    """
    db = get_db()
    task = db.get_task(tag)
    if not task:
    ...
    if task.task_type == TaskType.MULTI and task.tag_schema:
        tags_to_remove = [ts.tag for ts in task.tag_schema]
    else:
        tags_to_remove = [task.tag]
    
    # Determine folder path
    folder_path = request.folder if request.folder else "."
    
    # Remove each tag
    total_stats = {'total': 0, 'removed': 0, 'not_found': 0}
    tag_stats = {}
    
    for tag_to_remove in tags_to_remove:
        stats = classifier.remove_tag_from_folder(folder_path, tag_to_remove, dry_run=False)
        tag_stats[tag_to_remove] = stats
        total_stats['total'] = max(total_stats['total'], stats['total'])
        total_stats['removed'] += stats['removed']
        total_stats['not_found'] += stats['not_found']
    
    return {
        "message": f"Removed {len(tags_to_remove)} tag(s) from {folder_path}",
...
if __name__ == "__main__":
    ...
    def main(port: int, host: str):
        ...
        # Apply log filter to suppress polling endpoint logs
        uvicorn_access_logger = logging.getLogger("uvicorn.access")
        if not any(isinstance(f, PollingLogFilter) for f in uvicorn_access_logger.filters):
            uvicorn_access_logger.addFilter(PollingLogFilter())
